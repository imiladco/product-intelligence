...
| **Every mutation has a concrete red test** | **Fixed.** All 31 rows carry a targeted command and a named failure. Only #27 runs a whole suite, because the frontend has no targeted runner and the suite takes ~3s. Restore-before-next is stated, and the full suite runs once at the end |
